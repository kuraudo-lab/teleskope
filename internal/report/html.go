...
const liveStyles = ".live-progress { display:flex; align-items:flex-start; gap:10px; margin-bottom:16px; padding:12px 14px; border:1px solid var(--line); border-radius:12px; background:var(--panel-2); color:var(--muted); overflow-wrap:anywhere; } .live-progress-icon { color:var(--cyan); font-weight:800; flex:none; display:inline-grid; place-items:center; width:16px; height:16px; line-height:16px; } .live-progress-icon.spinning { border:2px solid var(--line); border-top-color:var(--cyan); border-radius:999px; color:transparent; animation:teleskope-spin .8s linear infinite; } @keyframes teleskope-spin { to { transform:rotate(360deg); } } .live-event-list { display:grid; gap:4px; max-height:calc(100vh - 190px); overflow:auto; font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace; font-size:12px; } .live-event { display:grid; grid-template-columns:86px 86px 92px 56px minmax(0,1fr); gap:8px; align-items:start; padding:5px 7px; border:1px solid var(--table-line); border-radius:8px; background:var(--live-event-bg); } .live-event-time, .live-event-kind, .live-event-source { color:var(--muted); } .live-event-kind { text-transform:uppercase; font-size:11px; font-weight:700; } .live-event-level { text-transform:uppercase; font-weight:700; } .live-event-info .live-event-level, .live-event-debug .live-event-level { color:var(--cyan); } .live-event-warn .live-event-level { color:var(--amber); } .live-event-error .live-event-level { color:var(--red); } .live-event-message { min-width:0; overflow-wrap:anywhere; } @media (max-width:700px) { .live-event { grid-template-columns:74px 1fr 52px; } .live-event-source, .live-event-message { grid-column:1 / -1; } }"
