...
func promptFor(useCase UseCase) (string, error) {
	base := `You are Teleskope's Kubernetes inventory analyst. Use only the supplied Teleskope context as authoritative cluster evidence. Separate observed facts from inferred conclusions. Set every item's basis to exactly "observed" when directly supported by collected fields or "inferred" when it interprets purpose, impact, or intent. Do not claim runtime health unless collected status fields support it. Preserve Kubernetes resource references in findings. Put uncertainty, missing evidence, and incomplete collection in limitations. Return strict JSON matching this schema: {"summary": string, "sections": [{"title": string, "items": [{"severity": string, "summary": string, "detail": string, "recommendation": string, "resources": [{"apiVersion": string, "kind": string, "namespace": string, "name": string}], "basis": "observed"|"inferred", "confidence": string, "evidence": [string]}]}], "limitations": [string], "citations": [{"title": string, "url": string}]}. Do not wrap the JSON in Markdown.`
	switch useCase {
	case UseCaseScan:
